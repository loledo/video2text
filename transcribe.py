...
def main():
    ...
    print(f"[video2text] Extracting audio from: {video_path}", file=sys.stderr)
    wav_path = extract_audio(video_path)

    try:
        print("[video2text] Running transcription...", file=sys.stderr)
        result = transcribe(
            wav_path,
            model_id=args.model,
            device=args.device,
    ...
    if args.output in ("txt", "both"):
        ...
        print(f"[video2text] Saved: {out_file}", file=sys.stderr)
